    ...
    $('body').on('click', '.edit-button', function(){
    	...
    	$('.resource-title').show();
    	// Hide label of title
    	current_plc.find('.resource-title').hide();
    	var control = getControl(current_plc.find('.resource-title').text());
    	current_plc.find('.resource-title').after(control);
    ...
    $('body').on('click', '#new-button-cancel', function(){
    	...
    	$(this).attr('id', 'new-button').text('New Resource Type');
    });
    	...
    	if(event.which === 13){
    		...
    		if(title != ''){
    			if(typeof row_val !== "undefined"){
    				var res_id = row_val.split('-')[1];
    			}else{
    				var res_id = 'none';
			    	$('#new-button-cancel').attr('class', 'btn btn-primary');
			    	$('#new-button-cancel').attr('id', 'new-button').text('New Resource Type');
    			}

				$('#result_output tbody').css('opacity', '0.6');
				$('.table-responsive .loading').fadeIn('fast');
         		
         		$.get('/internal-bkn/insert-resource-type?int_title='+ title +'&rid='+ res_id, function(result){
         			if(result != ''){
	    		...
	    		$('#confirm-msg-delete .modal-body p').text('Resource Type Title is required field, cannot empty.');
	...
	$('body').on('click', '#confirm-ok', function(){
		...
		$.get('/internal-bkn/delete-resource-type?did='+ did, function(deleted){
			if(deleted == 'success'){
	...
	$('body').on('click', 'ul.pagination a', function(ev){
		...
		var first_li   = '<li><a href="/internal-bkn/loading-resource-type-list?page=1">«</a></li>';
		var last_li    = '<li><a href="/internal-bkn/loading-resource-type-list?page='+ total_li +'" rel="next">»</a></li>';
			...
			for(var k=1; k<=total_li; k++){
				if($.isNumeric(active_val) == true){
					...
					if(parseInt(active_val) == k){
						...
						pagin_li += '<li><a href="/internal-bkn/loading-resource-type-list?page='+ k +'">'+ k +'</a></li>';
					}
				}else{
					pagin_li += '<li><a href="/internal-bkn/loading-resource-type-list?page='+ k +'">'+ k +'</a></li>';
				}
		...
		$.get('/internal-bkn/loading-resource-type-list?page='+ page, function(data){
			if(data != ""){
	...
	$('#find_resource_type').on('submit', function(e){
		e.preventDefault();
		...
		if(pagin != ''){
			...
			if(total_li > 0){
				for(var k=1; k<=total_li; k++){
					if(k == 1){
						pagin_li += '<li class="active"><a href="/internal-bkn/loading-resource-type-list?page='+ k +'">'+ k +'</a></li>';
					}else{
						pagin_li += '<li><a href="/internal-bkn/loading-resource-type-list?page='+ k +'">'+ k +'</a></li>';
					}
				}
    ...
    function resourceRow()
    {
    	var row_resource = '<tr bgcolor="#fed189"><td>&nbsp;</td>';
    		row_resource += '<td><input type="text" name="edit-title" class="form-control" id="control-title" placeholder="Resource Type Title"/></td>';
