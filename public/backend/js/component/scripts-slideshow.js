	...
	$('body').on('submit', '#find_slideshow', function (e){
		e.preventDefault();
		...
		if(pagin != ''){
			...
			if(total_li > 0){
				for(var k=1; k<=total_li; k++){
					if(k == 1){
						pagin_li += '<li class="active"><a href="/internal-bkn/loading-slideshow-list?page='+ k +'">'+ k +'</a></li>';
					}else{
						pagin_li += '<li><a href="/internal-bkn/loading-slideshow-list?page='+ k +'">'+ k +'</a></li>';
					}
				}
	...
	function is_validat_slideshow()
	{
		var status = true;
		if($('#slideTitle').val() == ''){
			$('#slideTitle').css('border', '1px solid red');
			$('.modal-title').text('Invalid user input');
			$('.modal-body p').text('Slideshow Title is required.');
			$('#confirm-msg-delete').modal('show');
			status = false;
		}
		...
		if($('#slide_upload').val() == ''){
			...
			$('.modal-title').text('Invalid user input');
			$('.modal-body p').text('Slideshow Image is required.');
			$('#confirm-msg-delete').modal('show');
			status = false;
		}
